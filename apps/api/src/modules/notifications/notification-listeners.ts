 ...
 * contract: every NotifType value has a `notifications.types.<camel>` key);
 * bodyKey carries the lifecycle nuance (the Phase 4 stub's workaround for
 * cancelled/completed/expired sharing enum values stays — `data.status`
 * holds the exact state).
 */
type ReqMapping = {
  ...
  onModuleInit(): void {
    ...
    for (const [event, mapping] of Object.entries(REQUEST_MAPPINGS)) {
      this.events.on(event as never, (payload: RentalRequestEventPayload) => {
        const targets =
          mapping.to === 'both'
            ? ([payload.ownerId, payload.tenantId] as const)
            : ([payload[mapping.to]] as const);
    ...
    // ── chat (Phase 5 gateway emits message.created) ──
    ...
    this.events.on('property.verified', (payload: PropertyEventPayload) => {
      void this.notifications.enqueue({
        ...
        titleKey: 'notifications.types.propertyVerified',
    ...
    this.events.on('property.rejected', (payload: PropertyEventPayload) => {
      void this.notifications.enqueue({
        ...
        titleKey: 'notifications.types.propertyRejected',
      ...
      for (const { userId } of fans) {
        await this.notifications.enqueue({
          ...
          data: {
            ...
            propertyId: payload.propertyId,
            ...(payload.title ? { propertyTitle: payload.title } : {}),
