...
describe('daily-stats rollup + views rebuild (§1.2.6)', () => {
  it('30 synthetic view days → 30 rollup rows; re-run is a no-op', async () => {
    ...
    // Anchor every event to the Asia/Tashkent calendar (the processor buckets
    // by Tashkent dates over a 30-day generate_series). A pure-UTC anchor put
    // "today" outside the window when the suite runs 00:00–05:00 local.
    const tkNow = new Date(Date.now() + 5 * 3_600_000);
    const dayAt = (offsetDays: number): Date =>
      new Date(
        Date.UTC(
          tkNow.getUTCFullYear(),
          tkNow.getUTCMonth(),
          tkNow.getUTCDate() - offsetDays,
          7, // 07:00 UTC == 12:00 Tashkent — mid-day, never a boundary race
          0,
        ),
      );
    for (let i = 0; i < 30; i++) {
      await prisma.propertyViews.create({
        data: { propertyId: property.id, sessionId: `s-${i}`, createdAt: dayAt(i) },
      });
    }
    await prisma.favorites.create({ data: { userId: fan.userId, propertyId: property.id, createdAt: dayAt(0) } });
    ...
    await prisma.messages.create({
      data: { conversationId: conversation.id, senderId: fan.userId, text: 'Salom', attachments: [], createdAt: dayAt(0) },
    });
