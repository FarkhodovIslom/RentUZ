...
function csp(): string {
  return [
    ...
    `img-src 'self' data: blob: https://*.supabase.co https://*.r2.cloudflarestorage.com https://*.amazonaws.com http://localhost:9000`,
    `connect-src 'self' ${API_HOSTS} wss://api.rentuz.uz`,
    `font-src 'self' data:`,
    `object-src 'none'`,
    `base-uri 'self'`,
