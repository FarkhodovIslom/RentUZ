...
16. **APP_GUARDs reach WS message handlers** (Phase 5 outcome): global guards (Throttle/JwtAuth/Suspended/Role) are applied to `@SubscribeMessage` handlers too, where `switchToHttp().getRequest()` is undefined — the JwtAuthGuard TypeError silently killed every gateway ack (acks never returned, `emitWithAck` timed out). All four guards now early-return `true` for `context.getType() !== 'http'` (socket auth is the gateway's ticket handshake; `message:send` has its own manual limiter).
17. **socket.io `connect` races `handleConnection`** (Phase 5): the client's `connect` event fires before the async ticket consume + room joins land, so an immediate `conversation:join` gets `NOT_PARTICIPANT` (userId not yet on `socket.data`). The gateway emits a **`ready`** event after the handshake work — clients wait for that before emitting anything. Also: invalid tickets connect first and are then force-disconnected (`io server disconnect`), so tests assert the late disconnect, not a `connect_error`.
18. **`prisma migrate dev` re-drops raw PostGIS indexes** (Phase 5): any new migration re-emits `DROP INDEX properties_location_gix` because `location` is `Unsupported` and the index is invisible to the schema (Phase 1 declared it in raw SQL on purpose). Every new migration file must re-add it (`CREATE INDEX IF NOT EXISTS`) — see `20260912115350_messages_cursor_index/migration.sql`. Same applies to other raw-SQL indexes (partial indexes survive — they're invisible but not dropped; only the GIST was re-emitted).
19. **engine.io Origin gate** (Phase 5): socket.io's `cors` option only covers the polling handshake — native WebSocket upgrades bypass it. A cross-origin socket connected successfully with a bad Origin until `allowRequest` was added to the gateway (rejects with a reason string, not an Error). The CSRF integration test asserts this.
...
- **Runtime feature flags (Phase 7)**: `AUTO_APPROVE_LISTINGS`, `AUTH_OTP_DEV_MODE`, `CHAT_ATTACHMENT_TTL_DAYS`, `NOTIFICATIONS_*_RETENTION_DAYS` read through a `FeatureFlagsService` — env default + Redis `flags:runtime:*` override with a 5 s in-process cache (no restart). The two dev-convenience booleans are **hard-false in production** at read time and `PATCH /admin/flags` refuses `true` there, so a Redis override can never bypass the boot assertions (which stay authoritative).
